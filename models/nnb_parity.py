...
@dataclass
class NNB_Parity(BaseNNB):
    estimator_name: str = "NNB-Parity"
    ...
    def _balance(self, X_rest, S, numpos_init):
        ...
        while disc > self.disc_threshold and self.enable_heuristic:
            ...
            if numpos < numpos_init:
                new = self.s_proba_[0, s_min] - self.delta * self.s_proba_[0, s_min]
                if new < 0:
                    break
                self.s_proba_[1, s_min] += self.delta * self.s_proba_[0, s_min]
                self.s_proba_[0, s_min] = new
            else:
                new = self.s_proba_[1, s_max] - self.delta * self.s_proba_[1, s_max]
                if new < 0:
                    break
                self.s_proba_[0, s_max] += self.delta * self.s_proba_[1, s_max]
